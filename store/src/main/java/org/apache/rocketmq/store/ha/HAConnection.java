...
public class HAConnection {
    ...
    class ReadSocketService extends ServiceThread {
        ...
        private boolean processReadEvent() {
            ...
            /**
             * 循环的方式进行读写，直到byteBuffer中没有剩余的空间
             */
            while (this.byteBufferRead.hasRemaining()) {
                try {
                    ...
                    if (readSize > 0) {
                         ...
                         * 如果读取的字节数小于0，表示连接处于半关闭状态, 返回false则意味着消息服务器将关闭该连接
                         */
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                try {
                     ...
                     * 如果slaveRequestOffset为0，则从当前commitlog文件最大偏移量进行传输，否则根据从服务器的拉取请求偏移量开始传输
                     */
                     ...
                     * 心跳包的长度为12字节(从服务器待拉取偏移量+size)，消息长度默认为0
                     * 避免长连接由于空闲被关闭，HA心跳包发送间隔可以通过haSendHeartbeatInterval放置，默认值为5S
                     *
                     *
                     * 2.如果上次数据未写完，则先传输一次的数据，如果消息还是未全部传输，则结束此次事件处理
                     */
                     ...
                     *
                     * HA服务端消息的传输一直重复上述操作，每次事件处理完成后等待1S
                     */
