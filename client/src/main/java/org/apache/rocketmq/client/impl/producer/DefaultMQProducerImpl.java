...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    public void start(final boolean startFactory) throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                // 向MQClientInstance注册，将当前生产者加入到管理中 ConcurrentMap<String/* group */, MQProducerInner> producerTable
    ...
    private SendResult sendDefaultImpl(
        ...
        // 消息发送之前，首先需要获取主题的路由信息，只有获取了这些消息，我们才知道消息要发送到具体的Broker节点
        ...
        // 如果没有找到相关的主题信息，路由信息，则抛出MQClientException
     ...
     * 如果路由信息未找到，就用默认的topic key TopicValidator.AUTO_CREATE_TOPIC_KEY_TOPIC
     * @param topic
     * @return
     */
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        /**
         * 如果生产者中缓存了topic路由信息，该路由信息中包含了消息队列，则直接返回路由信息
         */
    ...
    private SendResult sendKernelImpl(final Message msg,
         ...
         * 如果MQClientInstance的brokerAddrTable未缓存该Broker的信息，则从NameServer主动更新一下路由
         */
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                /**
                 * 如果消息体默认为4K，会对消息体采用ZIP压缩，并设置消息的系统标记为MessageSysFlag.COMPRESSED_FLAG
                 */
                int sysFlag = 0;
                 ...
                 * 消息扩展属性
                 * tag: 消息TAG, 用于消息过滤
                 * key: Message索引键，多个空格隔开，RocketMQ可以根据KEY快速检索到消息
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
         ...
         * 设置消息生产者组的目的是在查询事务消息本地事务状态时，从该生产者组中随机选择一个消费生产者即可
         * 然后通过同步方式向RocketMQ发送消息
         */
        SendResult sendResult = null;
         ...
         * 根据消息发送结果执行相应的操作
         *
         * 1.如果消息发送成功，则执行TransactionListener#executeLocalTransaction方法，该方法的职责
         * 是记录事务消息的本地事务状态，例如可以通过将消息唯一ID存储在数据中，并且该方法与业务代码处于同一个事务
         * 与业务事务要么一起成功，要么一起失败，为后续的事务状态回查提供唯一依据
         *
         * 2.如果消息发送失败，则设置本次事务状态为LocalTransactionState.ROLLBACK_MESSAGE
         * 3.结束事务，根据第二步返回的事务状态执行提交，回滚或者暂时不处理事务
         */
         ...
         * 应该返回LocalTransaction.UNKNOW, 事务消息的提交与回滚通过事务回查决定提交或回滚
         */
        try {
