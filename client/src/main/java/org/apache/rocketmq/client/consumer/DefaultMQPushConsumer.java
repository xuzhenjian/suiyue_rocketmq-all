...
public class DefaultMQPushConsumer extends ClientConfig implements MQPushConsumer {
    ...
    /**
     * consumeFromWhere, 根据消息进度从消息服务器拉取不到消息时，重新计算消费策略
     *
     * CONSUME_FROM_LAST_OFFSET: 从队列当前最大偏移量开始消费
     * CONSUME_FROM_FIRST_OFFSET: 从队列当前最小偏移量开始消费
     * CONSUME_FROM_TIMESTAMP: 从消费者启动时间戳开始消费
     *
     * 注意：如果从消息进度服务offsetStore读取到MessageQueue中的偏移量不小于0，则使用读取到的偏移量
     * 只有在读到的偏移量小于0，上述策略才会生效
     */
     ...
     */
    // 消费者最大的线程数， 由于消费者线程池使用无界队列，故消费者线程个数其实最多只有consumeThreadMin个
     ...
     * 表示如果消息处理队列中偏移量最大的消息与偏移量最小消息的跨度超过2000，则延迟50ms再拉取消息
     */
     ...
     * 消息并发消费时一次消费消息条数
     *
     * 也就是每次传入MessageListener#consumeMessage中的消息条数
     */
     ...
     */
    /**
     * 最大消息重试次数，如果消息消费次数超过maxReconsumeTimes还未成功，则将该消息转移到失败队列，等待被删除
     */
     ...
     */
    /**
     * 延迟将该队列的消息提交到消息消费线程的等待时间，默认延迟1S
     */
     ...
     * @param topic message topic 主题的名称
     * @return
     ...
     * @param fullClassName full class name,must extend org.apache.rocketmq.common.filter. MessageFilter 过滤类代码
